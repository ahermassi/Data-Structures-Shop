...
def find_path(source, dest):
    ...
    while queue:
        ...
        if (x, y) == dest:
            return reconstruct_path(source, dest, parent, count)
...
def reconstruct_path(source, dest, parent, count):
    ...
    return (path, count) if path[0] == source else (None, 0)  # If 'source' and 'dest' are connected, return the path
